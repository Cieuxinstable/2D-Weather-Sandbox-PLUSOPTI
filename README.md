# 2D Weather Sandbox PLUS (Optimized)

> **Fork by Cédric — based on [boreqq/2D-Weather-Sandbox-PLUS](https://github.com/boreqq/2D-Weather-Sandbox-PLUS)**
> Which is itself a fork of [niels747/2D-Weather-Sandbox](https://github.com/niels747/2D-Weather-Sandbox) — GPL-3.0 license.

### Changes in this fork
- **GUI panel width** reduced from 400px to 280px (less screen coverage)
- **Default IterPerFrame** reduced from 10 to 6 (better performance on mid-range GPUs)
- **Radar pixel sizes** increased from 8 to 10px by default (lighter rendering)
- **Radar refresh interval** increased from 2s to 3s by default (less GPU load)

All changes are adjustable in-game through the settings panel.
